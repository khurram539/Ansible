...
resource "aws_instance" "ansible_control_plane_server" {
  ...
  vpc_security_group_ids      = ["sg-03128daff9dfed41b", "sg-0e06cf3a9fea0966d"] # Add your security group IDs
  ...
  connection {
    ...
    private_key = file("/home/kkhoja/Code/Khurram-key.pem")    # Path to your local PEM key
    host        = self.public_ip                           # Public IP address of the instance
  }

  provisioner "file" {
    source      = "/home/kkhoja/Code/Khurram-key.pem"           # Path to your local PEM key
...
resource "aws_instance" "ansible_server" {
  ...
  tags = {
    Name = "AnsibleServer_${count.index + 1}"              # Instance name with index
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo 'AnsibleServer_${count.index + 1}' | sudo tee /etc/hostname", # Set the hostname permanently
...
resource "null_resource" "upload_inventory" {
  ...
  provisioner "file" {
    ...
    connection {
      type        = "ssh"
      user        = "ec2-user"
      private_key = file("/home/kkhoja/Code/Khurram-key.pem")
...
resource "null_resource" "update_hosts" {
  count = 3  # Total number of instances (1 control plane + 2 servers)
...
# ansible-playbook -i /home/ec2-user/inventory.ini /home/ec2-user/ping.yml --private-key /home/ec2-user/key.pem 